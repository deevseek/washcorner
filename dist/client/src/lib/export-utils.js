...
exports.exportServicesToExcel = exportServicesToExcel;
exports.exportIncomeReportToExcel = exportIncomeReportToExcel;
const XLSX = __importStar(require("xlsx"));
function exportTransactionsToExcel(transactions, options = {}) {
    // Default options
    ...
    const data = transactions.map((transaction, index) => {
        ...
        if (transaction.items && transaction.items.length > 0) {
            services = transaction.items.map((item) => item.serviceName || '-').join(', ');
        }
...
function exportServicesToExcel(services, options = {}) {
    // Default options
...
function exportIncomeReportToExcel(transactions, options = {}) {
    // Default options
