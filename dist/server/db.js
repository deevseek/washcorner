...
// Deklarasi fungsi untuk PostgreSQL
const initPostgres = () => {
...
if (useMySQL) {
    ...
    // Inisialisasi koneksi MySQL dan Drizzle
    const initMysql = async () => {
        try {
            const connection = await createMysqlConnection();
            return (0, mysql2_1.drizzle)(connection, { schema: mysqlSchema, mode: 'default' });
        }
        catch (error) {
            console.error('❌ Gagal inisialisasi Drizzle dengan MySQL:', error);
            throw error;
        }
    };
    // Inisialisasi database MySQL dengan fallback ke PostgreSQL
    (async () => {
        try {
            exports.db = db = await initMysql();
            ...
            if (process.env.DATABASE_URL) {
                ...
                try {
                    exports.db = db = initPostgres();
                }
                catch (pgError) {
                    console.error('❌ Gagal fallback ke PostgreSQL:', pgError);
                }
            }
        }
    })();
