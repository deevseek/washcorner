  ...
  try {
    ...
    try {
      ...
      if (users.length === 0) {
        ...
        // Dapatkan ID role admin dan kasir
        const [roleResults] = await connection.query('SELECT id FROM roles WHERE name = ?', ['admin']);
        const adminRoleId = roleResults[0]?.id || 1;
        
        const [cashierRoleResults] = await connection.query('SELECT id FROM roles WHERE name = ?', ['cashier']);
        const cashierRoleId = cashierRoleResults[0]?.id || 3;
        
        await connection.query(`
          INSERT INTO users (username, password, email, name, role_id) VALUES
          ('admin', '${adminPassword}', 'admin@washcorner.com', 'Administrator', ${adminRoleId}),
          ('cashier', '${cashierPassword}', 'cashier@washcorner.com', 'Kasir', ${cashierRoleId})
        `);
        console.log('User default berhasil dibuat');
      }
    } catch (err) {
      console.error('Error saat membuat data default:', err);
    }
