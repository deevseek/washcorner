// Script universal untuk menjalankan aplikasi Wash Corner - CommonJS version
// Berfungsi di semua sistem operasi terutama Windows
// Jalankan dengan: node start.cjs
...
try {
  ...
  if (isWindows) {
    ...
    try {
      // Salin vite-windows.ts ke vite.ts
      ...
      if (fs.existsSync(viteWindowsPath)) {
        ...
        console.log('✅ File vite.ts server diperbarui untuk Windows');
      }
      
      // Salin vite.config-windows.ts ke vite.config.ts
      const viteConfigWindowsPath = path.join(__dirname, 'vite.config-windows.ts');
      const viteConfigTargetPath = path.join(__dirname, 'vite.config.ts');
      
      if (fs.existsSync(viteConfigWindowsPath)) {
        fs.copyFileSync(viteConfigWindowsPath, viteConfigTargetPath);
        console.log('✅ File vite.config.ts diperbarui untuk Windows');
      }
    } catch (error) {
      console.error('❌ Error saat menyalin file vite:', error.message);
    }
